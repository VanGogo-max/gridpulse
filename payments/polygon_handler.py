...
POLYGONSCAN_API_KEY = None  # Ще се зареди от конфигурацията
...
def init_polygon_handler(config):
    ...
    """
    global OWNER_WALLET, POLYGON_RPC_URL, w3, POLYGONSCAN_API_KEY
...
def get_usdt_amount_from_tx(tx_hash):
    ...
    """
    try:
        # Използваме PolygonScan API за да получим детайли за транзакцията
        api_url = f"https://api.polygonscan.com/api?module=account&action=tokentx&txhash={tx_hash}&apikey={POLYGONSCAN_API_KEY}"
...
def get_wallet_balance(address=None):
    ...
    try:
        ...
        api_url = f"https://api.polygonscan.com/api?module=account&action=tokenbalance&contractaddress={USDT_CONTRACT}&address={address}&tag=latest&apikey={POLYGONSCAN_API_KEY}"
